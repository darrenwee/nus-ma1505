...
\begin{center}
     \Large{\textbf{$\mathbb{MA}1505$ Cheat Sheet}}\\\small{taken in AY 2015-2016 Semester I} \\
\end{center}
...
L'Hopital's rule can be applied multiple times in succession, e.g. if $\frac{f'(x)}{g'(x)}$ is still indeterminate.
...
Decide which function is $u(x)$ with the LIATE rule in this order of priority:

\resizebox{\linewidth}{!}{%
\begin{tabular}{|l@{ }c@{ }l@{ }c@{ }l@{ }c@{ }l@{ }c@{ }l|}
	\hline
	logarithmic & \multirow{2}{*}{$>$} & inverse trigo.& \multirow{2}{*}{$>$} & algebraic & \multirow{2}{*}{$>$} & trigo. 	& \multirow{2}{*}{$>$} & exponential \\
	functions 	& & functions 	& & functions	& & functions & & functions \\\hline
\end{tabular}}

\subsection{Area Bounded by Two Curves}
Area between two curves $f_2(x)$ and $f_1(x)$ where $f_1(x) \leq f_2(x)$ in $[a,b]$ is given by
...
If $|r| < 1$ then $ar^{n-1} \to 0$ as $n\to\infty$,
$$\sum_1^\infty ar^{n-1} = \frac{a}{1-r}$$
...
A Maclaurin series is a special case of a Taylor series where $a=0$, i.e. is a Taylor series expansion of a function about $0$
...
From now onwards, any math character in bold face is a vector, e.g. $\mathbf{i},\mathbf{u}$
...
The distance from a point $P(x_0,y_0,z_0)$ to a plane $\Pi:ax+by+cz=d$ is given by
$$\text{dist}(P,\Pi) =\frac{|ax_0+by_0+cz_0-d|}{\sqrt{a^2+b^2+c^2}}=\text{proj}_{\mathbf{n}}\vec{OP}$$
...
\begin{tabular}{rl}
	$-f(x) = f(-x)$ & function $f$ is odd \\
	$f(x) = f(-x)$	& function $f$ is even \\
\end{tabular}

\subsection{Computing the Fourier Series}
A periodic function of period $T$ can be represented by a Fourier series $f(x)$.
$$f(x) = a_0 + \sum_{n=1}^\infty (a_n\cos{nx}+b_n\sin{nx})$$

As the number of terms $n$ approaches infinity, the Fourier series begins to converge on the original function $f(x)$ more and more closely.
A perfect approximation of $f(x)$ can only occur when $n\to\infty$
...
Suppose $f(x)$ is a piecewise smooth periodic function on the interval $-L\leq x\leq L$ with a Fourier series $f_s(x)$
$$f_s(x) = \left\{ \begin{array}{ll}
			f(x) & \text{if } f \text{ is continuous at } x \\
			\frac{1}{2}[f(x^-)+f(x^+)] & \text{if } f \text{ is not continuous at } x \\
			\end{array}\right\}$$

This is useful when evaluating the sum of the Fourier coefficients $a_0$, $a_n$ and $b_n$ for some $x=k,k\in\mathbb{R}$ in $f(x)$
...
You can use the right-hand grip rule with the thumb pointed in the direction of the normal vector of $C$.
The curve has a positive orientation if your grip from knucle to fingertips progresses counter-clockwise, or negative if otherwise.
...
The curl and divergence operations can be expressed in terms of the del operator using cross and dot product.
...
{\hfill \texttt{For MA1505 taken in AY2015/16 Sem. 1 \\\hfill \LaTeX{} compiled \today.}}
\end{multicols}
\end{document}
